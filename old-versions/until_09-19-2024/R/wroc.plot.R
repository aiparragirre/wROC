...
#' @param print.auc If \code{TRUE}, the value of the area under the ROCw curve (AUCw) is printed (default \code{print.auc = TRUE}).
#' @param print.cutoff If \code{TRUE}, the value of the optimal cut-off point, and the corresponding weighted estimates of the sensitivity and specificity parameters are printed (default \code{print.cutoff = TRUE}).
#' @param col.cutoff Color in which the cut-off point is depicted. Default \code{col.cutoff = TRUE}.
#' @param cex.text Size with which the information of the AUCw and optimal cut-off point is printed. Default \code{cex.text = 0.75}.
#' @param round.digits Number of digits that will be employed when printing the information about the AUCw and optimal cut-off point.
#'
#' @return a graph
#'
#' @export
